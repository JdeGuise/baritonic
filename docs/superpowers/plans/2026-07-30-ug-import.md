...
**Tech Stack:** TypeScript 5 (strict), Vitest, Node 20+. Depends only on `@baritonic/music-core`.

## Global Constraints

- Depends on `@baritonic/music-core` only. No HTTP library — use global `fetch`.
...
```json
{
  ...
  "dependencies": {
    "@baritonic/music-core": "file:../music-core"
  },
  "devDependencies": {
    "typescript": "^5.6.0",
    "vitest": "^2.1.0"
...
```json
{
  "compilerOptions": {
    ...
    "paths": { "@baritonic/music-core": ["../music-core/src/index.ts"] }
  },
  "include": ["src", "test"]
}
```
...
**Interfaces:**
- Consumes: `tokens.ts` (Task 4); `Song`, `Section`, `Line`, `parseChord` from `@baritonic/music-core`.
...
**Interfaces:**
- Consumes: `parseNote`, `noteSemitone`, `LETTERS` from `@baritonic/music-core`.
...
**Interfaces:**
- Consumes: every module from Tasks 1-7; `detectKey`, `collectSymbols` from `@baritonic/music-core`.
