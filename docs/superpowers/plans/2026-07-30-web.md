...
**Tech Stack:** React 19, Vite 6, TypeScript 5 (strict), React Router 7, Vitest + Testing Library + jsdom. Depends on `@baritonic/music-core`.

## Global Constraints

- **The web app never imports `@baritonic/ug-import` or touches the database.** It talks to the API and to `music-core` only.
...
```json
{
  "compilerOptions": {
    ...
    "baseUrl": ".",
    "paths": { "@baritonic/music-core": ["../../packages/music-core/src/index.ts"] }
  },
  "include": ["src", "test"]
}
```
...
describe("App shell", () => {
  it("renders the brand and an import action", () => {
    ...
    expect(screen.getByRole("link", { name: /baritonic/i })).toBeInTheDocument();
...
**Interfaces:**
- Consumes: `ChordRef` from `@baritonic/music-core`.
...
**Interfaces:**
- Consumes: `Voicing` from `@baritonic/music-core`.
...
**Interfaces:**
- Consumes: `Song` from `@baritonic/music-core`.
...
import { parseChord, voicingsFor, fingerVoicing, spellChordTones, noteToString } from "@baritonic/music-core";
...
```bash
cd apps/server && rm -rf /tmp/baritonic-e2e && \
  DATA_DIR=/tmp/baritonic-e2e PORT=4200 npm start
```
...
Then stop the server and clean up: `rm -rf /tmp/baritonic-e2e`.
