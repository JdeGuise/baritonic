...
**Interfaces:**
- Consumes: `keyDelta`, `transposeChordSymbol`, `parseChord` from `@baritonic/music-core`.
...
cd ../server && rm -rf /tmp/baritonic-edit && DATA_DIR=/tmp/baritonic-edit PORT=4201 npm start
```
...
```bash
pkill -f "src/server.ts"; rm -rf /tmp/baritonic-edit
```
