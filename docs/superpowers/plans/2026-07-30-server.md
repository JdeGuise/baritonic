...
**Tech Stack:** TypeScript 5 (strict), Vitest, Node 26+, Express 5, and **`node:sqlite` from the Node standard library**. Depends on `@music-ui/music-core` and `@music-ui/ug-import`.
...
```json
{
  ...
  "dependencies": {
    ...
    "@music-ui/ug-import": "file:../../packages/ug-import",
    "express": "^5.0.0"
  },
  "devDependencies": {
    "@types/express": "^5.0.0",
...
```json
{
  "compilerOptions": {
    ...
    "baseUrl": ".",
    "paths": {
      "@music-ui/music-core": ["../../packages/music-core/src/index.ts"],
      "@music-ui/ug-import": ["../../packages/ug-import/src/index.ts"]
    }
  },
  "include": ["src", "test"]
}
...
**Interfaces:**
- Consumes: `Song`, `Line` from `@music-ui/music-core`.
...
**Interfaces:**
- Consumes: `openDatabase`, `migrate` (Task 2); `Song` from `@music-ui/music-core`; `ImportResult` from `@music-ui/ug-import`.
...
**Interfaces:**
- Consumes: `ImportError`, `describeFailure` from `@music-ui/ug-import`.
...
**Interfaces:**
- Consumes: `createSongRepo` (Task 4), `createOverrideRepo` (Task 5), `applyOverrides` (Task 3), `importFromUrl`, `importFromText` from `@music-ui/ug-import`, `HttpError` (Task 6).
...
**Interfaces:**
- Consumes: `createOverrideRepo` (Task 5), `createSongRepo` (Task 4), `HttpError` (Task 6), `parseChord` from `@music-ui/music-core`.
...
Run: `cd apps/server && DATA_DIR=/tmp/music-ui-smoke PORT=4199 timeout 5 npm start`
Expected: logs the listening address, the database path with `schema v1`, and `web assets: not built — API only`. Then clean up: `rm -rf /tmp/music-ui-smoke`.
...
**Type consistency.** Verified across tasks: `ChordOverride` is defined once in `overlay.ts` and reused by `repo/overrides.ts` and both routers; `ChordPosition` is used identically in `remove` and the delete route; `SongUpdate` fields match between repo and the PATCH handler; `AppDeps` gains `staticDir` in Task 9 and is consumed only in `app.ts` and `server.ts`; `FetchLike` threads from `@music-ui/ug-import` through `AppDeps` to `importFromUrl` unchanged.
